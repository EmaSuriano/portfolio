...
## Why won't Typescript always cover you? 🕵
...
Many validation libraries force you to define an `schema` where you can describe the structure to evaluate. Given that schema you can create the validation function which is going to be used in your code.

The declaration of your schema will always depend on the library you are using, therefore I always recommend to check the official docs. For the case of `ajv`, it's enforce you to declare in an Object Style, where each property has to provide the `type` of it and, in adittion, it's possible to set custom checker for these values, like `minLength` for any `array` or `string`.

This method provides:

- **A standardized way to create validators and checks**: the idea behind the `schema` is to have only **one** way to check for specific conditions inside your application. Specially in Javascript, where there are many ways to accomplish the same task, such as checking the `length` of an array. This quality is great to improve the communication and collaboration inside a team.
...
- **Introduction of new Syntax**: when a team take the decision of adding a new library, the difficulty degree to understand the whole codebase grows too. Every contributor has to know about the validator schema in order to understand how to make a change on it.
- **Validators and Types are not in Sync**: the definition of the `schema` and the `ExampleType` Type are totally disconnected, which means that every time you make a change inside the `ExampleType`, you have to manually reflect it inside the `schema`. Depending on how many validator, you have this task can be a quite tedious.

One small comment regarding keeping in Sync Validators and Types, there are some open source project that address this issue, such as [json-schema-to-typescript](https://www.npmjs.com/package/json-schema-to-typescript), which can generate a Type definition from an existing Schema. Then this won't be consider problem.
...
The method of Dynamic Types Validator allows a Type to generate a validator from his definition. Now they present a relation of dependency between, where a Validator depends entirely on a Type, preventing any mismatch between structures.
...
In order to generate these Validators, I found an amazing Open Source project called [`typescript-json-validator`](https://github.com/ForbesLindesay/typescript-json-validator), made by [@ForbesLindesay](https://twitter.com/ForbesLindesay). The description of the repository is:

> Automatically generate a validator using JSON Schema and AJV for any TypeScript type.

For the test, let's re-use the `ExampleType` definition, which now has been moved to a separated file inside the `types` folder.
...
This library expose a handy CLI that can be called from anywhere, and given a file path and the name of the Type, it will generate in the same location as the file a new file with the Validator code.
...
The resulting validator can be very long file, so let's take a look piece by piece:

1. Creating the instance of `ajv` and some default configuration.
...
2. Definition of the `schema` from the Type. This is by far the part I loved the most from this library.
...
3. Export validation function using the generated `schema`. It takes care also of the throwing an Exception in case there is an error.
...
In order to use the validator, you just need to import from the respective path and call it. Be aware that this function is already checking if there was any errors inside the object, therefore it's not needed to add an `if` statement here, making the code much cleaner.
...
This library use `ajv` under the hood to create the Validator function, which means that you can make use of all nice features it provides, such as custom validation for types.
...
The property `answer` presents now two more attributes that will check if the `number` is between 0 and 100. In the case for `email`, it will check if the `string` value belong to a valid email address.

As these annotations are wrapped inside comments section, they don't present any conflict with the Typescript compiler.

## Making it part of your workflow

This method is based on the idea that the developer will run the CLI command and generate the validators, otherwise it exists the possibility that the schema was generated with an older version of the Type and then it can present mis matches.

Fixing this issue is actually quite easy, you have to simple add a script that will be executed before your code will run. You can call it `prebuild` or `prestart`, and this is how your `package.json` can look like:
...
As a last recommendation, I recommend ignoring any `validator.ts` file from your project, because there is no point on committing these files to your repository as they are going to be generate it every time you start your project.

## My experience with this approach 🙋‍♂️

About 2 month ago, I open sourced one of my side projects called [`gatsby-starter-linkedin-resume`](https://github.com/EmaSuriano/gatsby-starter-linkedin-resume). This is description of it:
...
At the beginning of this article, I mentioned that it's always recommended to check your external sources. For this project they are:
...
Both Types present similarities in terms of variable names, but their internal structure differ. This is why it's necessary to transform from one structure to the other on the first flow.
